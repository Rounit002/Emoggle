...
export function useExpressionScorer(
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Skip the heavy inference + setState when we're not actively
      // scoring (lobby, countdown, dueling, results). The rAF keeps
      // spinning so we can resume the moment `active` flips back on,
      // but we don't pay the MediaPipe + React re-render cost every
