...
export default function UploadJudge({ onBack }: UploadJudgeProps) {
  const { sessionToken } = useUserProfile();
  ...
  const handleFile = useCallback((file: File) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setJudgeError("Please choose an image smaller than 5 MB.");
      return;
    }
  ...
  const handleAnalyze = async () => {
    if (!preview) return;
    if (!sessionToken) {
      setJudgeError("A secure session could not be created. Please refresh and try again.");
      return;
    }
    const base64 = preview.replace(/^data:image\/\w+;base64,/, "");
    if (base64.length > 900_000) {
      setJudgeError("This image is too large to analyze securely. Please use a smaller image.");
      return;
    }
    setPhase("analyzing");
    setJudgeError(null);
    ...
    try {
      const res = await fetch(`${SIGNALING_URL}/api/judge`, {
        method: "POST",
        credentials: "include",
        headers: {
